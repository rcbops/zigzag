...
def single_passing_tempest_with_test_execution_parameters(tmpdir_factory, default_global_properties, default_testcase_properties):
    ...
    junit_xml = \
        ...
        <testsuite errors="0" failures="0" name="pytest" skips="0" tests="5" time="1.664">
            {global_properties}
            <testcase classname="tempest.api.identity.admin.v3.test_endpoints.EndPointsTestJSON"
            name="test_server_basic_ops[compute,id-7fff3fb3-91d8-4fd0-bd7d-0204f1f180ba,network,smoke]" time="0.236"/>
                {testcase_properties}
            </testcase>
        </testsuite>
...
@pytest.fixture(scope='module')
def single_passing_with_test_execution_parameters(tmpdir_factory, default_global_properties, default_testcase_properties):
    """A single passing test that does not have 'system-out' or 'system-err' testcase elements."""
...
class TestZigZagTestLog(object):
    ...
    def test_lookup_test_execution_parameters_not_found(self, single_passing_xml, mock_zigzag):
        ...
        # Test
        tl = ZigZagTestLogs(zz)[0]   # Create a new TestLog object through the ZigZagTestLogs public class
        assert tl.test_execution_parameters == []

    def test_lookup_test_execution_parameters_found(self, single_passing_with_test_execution_parameters, mock_zigzag):
        """Test for _lookup_ids
        Ask for a test ID that does not exist yet
        """
        ...
        mock_zigzag(search_response)
        zz = ZigZag(single_passing_with_test_execution_parameters, TOKEN, PROJECT_ID, TEST_CYCLE)
        zz.parse()

        # Test
        tl = ZigZagTestLogs(zz)[0]   # Create a new TestLog object through the ZigZagTestLogs public class
        assert tl.test_execution_parameters == ['testinfra', 'host0']
...
class TestZigZagTestLogsTempest(object):
    ...
    def test_sample_tempest_xml_with_execution_parameters(self, single_passing_tempest_with_test_execution_parameters, mock_zigzag):
        """Verify that we can parse xml generated by tempest"""

        # Setup
        mock_zigzag()
        zz = ZigZag(single_passing_tempest_with_test_execution_parameters, TOKEN, PROJECT_ID, TEST_CYCLE)
        zz.test_runner = 'tempest'
        zz.parse()

        # Test
        tls = ZigZagTestLogs(zz)
        assert tls.test_execution_parameters == ['compute','id-7fff3fb3-91d8-4fd0-bd7d-0204f1f180ba','network','smoke']
